...
#include "Tool.cpp"        
using namespace std;
struct BurstRecord_P2S_DT {
    uint32_t flowLabel;
    int period; 
    BurstRecord_P2S_DT(uint32_t f = 0, int p = 0) : flowLabel(f), period(p) {}
    bool operator<(const BurstRecord_P2S_DT& other) const {
...
public:
    ...
    int cur_est; 
    int priority;
    CellOfBkt_P2S_DT(uint32_t f = 0, int p_est = 0, int c_est = 0, int p = 0)
...
public:
    ...
    set<BurstRecord_P2S_DT> detected_burst_records; 
    int burstThreshold;
    P2S_DT(int bitmapLen, int numOfBkt, int sizeOfBkt, int d, int w, double highBitmapRatio, int gamma, double ratioThreshold, int burstThreshold)
        : gamma(gamma), numOfBkt(numOfBkt), sizeOfBkt(sizeOfBkt), d(d), w(w), ratioThreshold(ratioThreshold), burstThreshold(burstThreshold) {
    ...
    void update(uint32_t src, uint32_t dst, int priority) {
        ...
        if (priority > gamma * 0.75) { 
            ...
            if (!flag) return; 
            estIncrement = static_cast<int>(static_cast<double>(highBitmapLen) / zeroNumOfBitmapHigh);
            if (priority > gamma * 0.75) { 
                ...
                if (posInTheBkt > -1) {
                    ...
                    if (bktArray[idx4Bkt].cellArray.size() == static_cast<size_t>(sizeOfBkt)) {
                        ...
                        if (priority > bktArray[idx4Bkt].cellArray[min_idx].priority) {
                            CellOfBkt_P2S_DT tempCell(src, 0, estIncrement, priority);
    ...
    void endPeriod(int currentPeriodIdx) {
        for (size_t i = 0; i < numOfBkt; ++i) {
            for (size_t j = 0; j < bktArray[i].cellArray.size(); ++j) {
                CellOfBkt_P2S_DT& cell = bktArray[i].cellArray[j];
                if (cell.priority > 0.75 * gamma) {
                    if (cell.cur_est > burstThreshold && static_cast<double>(cell.cur_est)  > ratioThreshold * cell.pre_est) {
                        detected_burst_records.insert(BurstRecord_P2S_DT(cell.flowLabel, currentPeriodIdx));
                    }
                }
            }
        }
        ...
        fill(lowBitmap.begin(), lowBitmap.end(), false);
        fill(highBitmap.begin(), highBitmap.end(), false);
        zeroNumOfBitmapLow = lowBitmapLen;
        zeroNumOfBitmapHigh = highBitmapLen;
    }
    int estimate(uint32_t src) {
        ...
        int posInTheBkt = bktArray[idx4Bkt].posOfFlowInBkt(src);
        if (posInTheBkt > -1) {
            return bktArray[idx4Bkt].cellArray[posInTheBkt].cur_est;
        } else {
    ...
    const set<BurstRecord_P2S_DT>& getDetectedBurstRecords() const {
...
public:
    ...
    set<BurstRecord_P2S_DT> true_burst_records; 
    Experiment_P2S_DT(int bitmapLen, int numOfBkt, int sizeOfBkt, int d, int w, double highBitmapRatio, int gamma, double Th, int burstThreshold)
        : gamma(gamma), burstThreshold(burstThreshold), sketch(bitmapLen, numOfBkt, sizeOfBkt, d, w, highBitmapRatio, gamma, Th, burstThreshold) {}
        ...
        for (int period_idx = 0; period_idx < T; ++period_idx) {
            ...
            for (uint32_t flow_label : all_active_flows_in_window) {
                ...
                if (current_spread > burstThreshold && static_cast<double>(current_spread) > sketch.ratioThreshold * previous_spread) {
                    true_burst_records.insert(BurstRecord_P2S_DT(flow_label, period_idx));
                }
            }
    ...
    void performance_burst(vector<vector<double>>& results, int idx) {
        ...
        const set<BurstRecord_P2S_DT>& detected_bursts = sketch.getDetectedBurstRecords();
        vector<BurstRecord_P2S_DT> true_bursts_vec(true_burst_records.begin(), true_burst_records.end());
        vector<BurstRecord_P2S_DT> detected_bursts_vec(detected_bursts.begin(), detected_bursts.end());
        vector<BurstRecord_P2S_DT> common_bursts_vec;
...
vector<double> Exp_P2S_DT(double alpha, int gamma, double wholeMemory, string priority_file_name, double burstTh, int T, int burstThreshold, string filename) {
    cout.precision(10); 
    ...
    Experiment_P2S_DT exp(bits, numOfBkt, sizeOfBkt, d, w, k, gamma, burstTh, burstThreshold); 
