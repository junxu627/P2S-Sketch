...
public:
    ...
    Experiment_BF_MC(int bitmapLen, int k, int numOfLayers, int sizeOfBkt, int d, int w, int gamma_val, int maxPriority_val)
        : gamma(gamma_val), 
            sketch(bitmapLen, k, numOfLayers, sizeOfBkt, d, w, maxPriority_val) {}
    ...
    void estimate_flows() {
        ...
        for (auto const& [src, dst_set] : real_set_dict) {
            ...
            if (count % update_interval == 0 || count == total_flows) {
                ...
                cout << "[Messages] Estimating flows): " << progress << "%" << flush;
            }
        }
        cout << endl;
        cout << "[Messages] Estimation complete" << endl << endl;
    }
    void performance(vector<vector<double>>& results, int idx) {
        cout << "[Messages] Evaluating performance" << endl;
        if (real_dict.empty()) {
            cout << "[Messages] No data for performance evaluation." << endl;
            return;
        }
        ...
        cout << "countHigh " << countHigh << endl;
        cout << "highPriorityFlowNum " << highPriorityFlowNum << endl;
        double pr = 1.0; 
    ...
    void pefmance2() {
        cout << "[Messages]: Evaluating pefmance2 (recorded high-priority)" << endl;
            if (real_dict.empty()) {
            cout << "[Messages]: No data for pefmance2 evaluation." << endl;
            return;
        }
        double highPriorityErrorSUM = 0;
        int highPriorityFlowNum = 0;
        ...
        cout << "ARE for recorded high-priority flows only: " << fixed << setprecision(6) << are_val << endl;
    }
};
vector<double> Exp_BF_MC(double alpha, int gamma, double wholeMemory, string priority_file_name) {
    cout.precision(10);
    ...
    int maxPriority_main = 8;
    Experiment_BF_MC exp(bits_main, k_main, numOfLayers_main, sizeOfBkt_main, d_main, w_main, gamma, maxPriority_main);
