...
void generate_prioritized_data(const string& InputConvertedFilename, double alpha, int gamma, const string& OutputPrioritizedFilename) {
    ...
    string line;
    if (!infile.is_open()) {
        cerr << "error:" << InputConvertedFilename << endl;
        return;
    }
    if (!outfile.is_open()) {
        cerr << "error:" << OutputPrioritizedFilename << endl;
        return;
    }
    int line_count = 0;
...
void calculate_flow_cardinalities(const string& InputConvertedFilename, const string& OutputFlowCardinalitiesFilename) {
    ...
    if (!outfile.is_open()) {
        cerr << "error:" << OutputFlowCardinalitiesFilename << endl;
        return;
    }
...
void generate_volume_dependent_prioritized_data(
    ...
    if (!card_infile.is_open()) {
        cerr << "error:" << InputCardinalitiesFilename << endl;
        return;
    }
    string line;
    ...
    if (!original_infile.is_open()) {
        cerr << "error:" << InputOriginalConvertedFilename << endl;
        return;
    }
    if (!outfile.is_open()) {
        cerr << "error:" << OutputPrioritizedFilename << endl;
...
void generate_volume_dependent_prioritized_data_new(
    ...
    cout << "\n[Debug Info]: Zipf Distribution Probabilities and Flow Count Thresholds:\n";
