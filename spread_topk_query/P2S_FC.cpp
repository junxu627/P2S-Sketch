...
struct CellOfTopk_P2S_FC {
    uint32_t flowLabel;
    float est; 
    int priority;
    CellOfTopk_P2S_FC(uint32_t f = 0, float e = 0, int p = 0) : flowLabel(f), est(e), priority(p) {}
    bool operator<(const CellOfTopk_P2S_FC& other) const {
...
public:
    ...
    vector<CellOfTopk_P2S_FC> topkArray;
    int k_topk; 
    P2S_FC(int bitmapLen, int numOfBkt, int sizeOfBkt, int d, int w, int gamma, double base, double step, int k_topk_val)
        : gamma(gamma), bitmapLen(bitmapLen), numOfBkt(numOfBkt), sizeOfBkt(sizeOfBkt), d(d), w(w), k_topk(k_topk_val) {
    ...
    void updateTopk(uint32_t flowLabel, float spread, int priority) {
        ...
        if (existing_idx != -1) {
            ...
            if (topkArray.size() < static_cast<size_t>(k_topk)) {
                ...
                topkArray.back() = CellOfTopk_P2S_FC(flowLabel, spread, priority); 
                sort(topkArray.begin(), topkArray.end()); 
            }
        }
    }
    ...
    vector<CellOfTopk_P2S_FC> getTopkSuperSpread() const {
        return topkArray;
    }
};
...
public:
    ...
    Experiment_P2S_FC(int bitmapLen, int numOfBkt, int sizeOfBkt, int d, int w, int gamma, double base, double step, int k_topk)
        : gamma(gamma), sketch(bitmapLen, numOfBkt, sizeOfBkt, d, w, gamma, base, step, k_topk) {}
    ...
    void new_performance_topk(vector<vector<double>>& results, int idx) {
        vector<CellOfTopk_P2S_FC> true_high_priority_flows;
        for (const auto& pair : real_dict) { 
        ...
        sort(true_high_priority_flows.begin(), true_high_priority_flows.end(),
                  [](const CellOfTopk_P2S_FC& a, const CellOfTopk_P2S_FC& b) {
                      return a.est > b.est; 
                  });
        vector<CellOfTopk_P2S_FC> true_topk_flows_extended;
        ...
        vector<CellOfTopk_P2S_FC> estimated_topk_flows = sketch.getTopkSuperSpread();
...
vector<double> Exp_P2S_FC(double alpha, int gamma, double wholeMemory, string priority_file_name, int k_topk) {
    cout.precision(10);
    ...
    double base = 100; 
    double step = 2; 
    Experiment_P2S_FC exp(bits, numOfBkt, sizeOfBkt, d, w, gamma, base, step, k_topk);
