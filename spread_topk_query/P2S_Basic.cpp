...
struct CellOfTopk_P2S_Basic {
    uint32_t flowLabel;
    float est; 
    int priority;
    CellOfTopk_P2S_Basic(uint32_t f = 0, float e = 0, int p = 0) : flowLabel(f), est(e), priority(p) {}
    bool operator<(const CellOfTopk_P2S_Basic& other) const {
...
public:
    ...
    vector<CellOfTopk_P2S_Basic> topkArray;
    int k_topk; 
    P2S_Basic(int bitmapLen, int numOfBkt, int sizeOfBkt, int d, int w, int gamma, int k_topk_val)
        : gamma(gamma), bitmapLen(bitmapLen), zeroNumOfBitmap(bitmapLen),
          numOfBkt(numOfBkt), sizeOfBkt(sizeOfBkt), d(d), w(w), k_topk(k_topk_val) { 
    ...
    void updateTopk(uint32_t flowLabel, float spread, int priority) {
        ...
        if (existing_idx != -1) {
            ...
            if (topkArray.size() < static_cast<size_t>(k_topk)) {
                ...
                topkArray.back() = CellOfTopk_P2S_Basic(flowLabel, spread, priority); 
                sort(topkArray.begin(), topkArray.end()); 
            }
        }
    }
    ...
    void update(uint32_t src_flow_label, uint32_t dst_flow_label, int priority) {
        ...
        if (priority > gamma * 0.6) { 
            ...
            if (posInTheBkt > -1) {
                ...
                updateTopk(src_flow_label, bktArray[idx4Bkt].cellArray[posInTheBkt].est, priority);
            } else {
                if (bktArray[idx4Bkt].cellArray.size() == static_cast<size_t>(sizeOfBkt)) {
                    ...
                    if (priority > bktArray[idx4Bkt].cellArray[min_idx].priority) {
                        ...
                        updateTopk(src_flow_label, estIncrement, priority);
                    } else {
    ...
    vector<CellOfTopk_P2S_Basic> getTopkSuperSpread() const {
        return topkArray;
    }
};
...
public:
    ...
    Experiment_P2S_Basic(int bitmapLen, int numOfBkt, int sizeOfBkt, int d, int w, int gamma, int k_topk)
        : gamma(gamma), sketch(bitmapLen, numOfBkt, sizeOfBkt, d, w, gamma, k_topk) {}
    ...
    void performance_topk(vector<vector<double>>& results, int idx) {
        vector<CellOfTopk_P2S_Basic> true_high_priority_flows;
        for (const auto& pair : real_dict) { 
        ...
        sort(true_high_priority_flows.begin(), true_high_priority_flows.end(), 
                  [](const CellOfTopk_P2S_Basic& a, const CellOfTopk_P2S_Basic& b) {
                      return a.est > b.est; 
                  });
        vector<CellOfTopk_P2S_Basic> true_topk_flows;
        size_t effective_k_true = min(static_cast<size_t>(sketch.k_topk), true_high_priority_flows.size());
        for (size_t i = 0; i < effective_k_true; ++i) {
            true_topk_flows.push_back(true_high_priority_flows[i]);
        }
        set<uint32_t> true_topk_labels;
        for (const auto& cell : true_topk_flows) {
            true_topk_labels.insert(cell.flowLabel);
        }
        vector<CellOfTopk_P2S_Basic> estimated_topk_flows = sketch.getTopkSuperSpread();
        ...
        set_intersection(true_topk_labels.begin(), true_topk_labels.end(),
        ...
        double PR = 0.0;
        if (!estimated_topk_flows.empty()) {
            PR = static_cast<double>(num_common_flows) / estimated_topk_flows.size();
        }
        double AAE_sum = 0.0;
        double ARE_sum = 0.0;
    ...
    void new_performance_topk(vector<vector<double>>& results, int idx) {
        ...
        vector<CellOfTopk_P2S_Basic> true_topk_flows_extended;
...
vector<double> Exp_P2S_Basic(double alpha, int gamma, double wholeMemory, string priority_file_name, int k_topk) {
    cout.precision(10); 
    ...
    Experiment_P2S_Basic exp(bits, numOfBkt, sizeOfBkt, d, w, gamma, k_topk);
